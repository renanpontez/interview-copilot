...
import { deleteJobCv } from "@/lib/storage/cv";
import type { Job, JobStatus, Scenario } from "@/lib/storage/types";
...
export default function JobDetailPage({
  ...
  useEffect(() => {
    ...
    setScenarios(getScenariosForJob(id));
  }, [id]);
  ...
  function addScenario() {
    ...
    saveScenario(sc);
    setScenarios(getScenariosForJob(job.id));
  }

  function updateScenario(sc: Scenario, patch: Partial<Scenario>) {
    const next = { ...sc, ...patch };
    saveScenario(next);
