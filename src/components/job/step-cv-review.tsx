...
import { getSettings, getProfile } from "@/lib/storage/local";
import { blobToFile, cloneBaseToJob, deleteJobCv, getBaseCv, getJobCv, setJobCv } from "@/lib/storage/cv";
import { recordUsage } from "@/lib/hooks/use-track-cost";
import type { DocEditorHandle } from "@/components/resume/doc-editor";
import type { Job } from "@/lib/storage/types";
...
export function StepCvReview({ job, onUpdate, onNext, onSkip, onBack }: StepCvReviewProps) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const stored = await getJobCv(job.id);
    if (stored) {
      setFile(blobToFile(stored.blob, stored.fileName));
      setFileName(stored.fileName);
    } else {
      setFile(null);
    }
    setBaseAvailable(Boolean(await getBaseCv()));
    setLoading(false);
  }, [job.id]);
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    await setJobCv(job.id, selected, selected.name);
    setFile(selected);
  ...
  async function handleCloneBase() {
    const stored = await cloneBaseToJob(job.id);
    if (!stored) return;
    setFile(blobToFile(stored.blob, stored.fileName));
    setFileName(stored.fileName);
    onUpdate({ hasTailoredCv: true });
  ...
  async function handleSave() {
    ...
    try {
      ...
      await setJobCv(job.id, blob, fileName);
      onUpdate({ hasTailoredCv: true });
      setStatus("saved");
  ...
  async function handleAnalyze() {
    ...
    if (!settings.apiKey) { alert("Set your API key in Settings first."); return; }
    ...
    try {
      ...
      if (data.usage) recordUsage(data.usage as AIUsage, "analyze-cv");
