...
import { speedUpAudio } from "@/lib/audio/speed-up";
...
export function AudioRecorder({
  ...
  async function transcribeWithWhisper(blob: Blob) {
    ...
    try {
      // Speed up audio 1.5x to reduce Whisper cost (~33% savings)
      let audioToSend: Blob;
      try {
        audioToSend = await speedUpAudio(blob, 1.5);
      } catch {
        // Fallback to original if speedup fails
        audioToSend = blob;
      }
      const ext = audioToSend.type === "audio/wav" ? "recording.wav" : "recording.webm";
      const fd = new FormData();
      fd.append("audio", audioToSend, ext);
      fd.append("apiKey", apiKey);
      if (whisperContext) fd.append("context", whisperContext);
